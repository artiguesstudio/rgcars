...
function prefillFromUrl() {
  ...
  if (vehicleTitle) {
    form.vehicle_reference.value = vehicleTitle;
...
async function handleSubmit(event) {
  ...
  try {
    const vehicleReference = window.RGShared.normalizeLeadText(form.vehicle_reference.value) || form.dataset.vehicleTitle || null;
    const note = window.RGShared.normalizeLeadText(form.notes.value);
    const result = await window.RGShared.submitServiceLead({
      serviceType: form.service_type.value || 'peritaje_precompra',
      source: form.source.value || 'servicio_peritaje_precompra',
      ...
      vehicleTitle: vehicleReference || null,
      metadata: {
        inspection_vehicle: vehicleReference || null,
      },
    });
...
document.getElementById('peritajeReset')?.addEventListener('click', resetForm);
prefillFromUrl();
